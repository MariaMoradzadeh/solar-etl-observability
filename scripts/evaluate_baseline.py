...
def load_edge(cfg, scenario, seed, w):
    p = (
...
def load_baseline(cfg: dict, scenario: str, seed: int, w: int) -> pd.DataFrame:
    p = Path(cfg["paths"]["results_dir"]) / "edge" / scenario / f"seed_{seed}" / f"w_{w}.parquet"
...
def main():
    ...
    out_root = Path(cfg["paths"]["results_dir"]) / "metrics_baseline"
    ensure_dir(out_root)
